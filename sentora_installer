...
if [[ $1 = PHP* ]]; then
	...
	if [[ $1 = "PHP56" ]]; then
        ...
        # Start PHP 5.6 & tools install here
            
        # Install PHP 5.6 & Repos
        if [[ "$OS" = "Ubuntu" || "$OS" = "debian" ]]; then
		...
		$PACKAGE_INSTALLER python-software-properties
                ...
                # Install PHP 5.6 and install modules
                ...
                if [ ! -f /etc/php/5.6/apache2/conf.d/20-mcrypt.ini ]
                        then
                    ...
                    bash -c "echo extension=mcrypt.so > /etc/php/5.6/mods-available/mcrypt.ini"
                    ln -s /etc/php/5.6/mods-available/mcrypt.ini /etc/php/5.6/apache2/conf.d/20-mcrypt.ini
                fi		
                
                # Set PHP 5.6 as system default in case upgrade to PHP 7.4+
                update-alternatives --set php /usr/bin/php5.6
                
                # Enable Apache mod_php5.6
                a2enmod php5.6  
	...
	if [[ "$OS" = "CentOs" ]]; then
		...
		if [[ "$VER" = "18.04" ]]; then
			...
			# Create php-mcrypt modules file
			touch /etc/php/5.6/mods-available/mcrypt.ini
			echo 'extension=mcrypt.so' >> /etc/php/5.6/mods-available/mcrypt.ini
						
			# Create links to activate PHP-mcrypt
			ln -s /etc/php/5.6/mods-available/mcrypt.ini /etc/php/5.6/apache2/conf.d/20-mcrypt.ini
			ln -s /etc/php/5.6/mods-available/mcrypt.ini /etc/php/5.6/cli/conf.d/20-mcrypt.ini
...
if [[ "$VER" = "16.04" || "$VER" = "18.04" || "$VER" = "20.04" || "$VER" = "7" || "$VER" = "8" ]]; then
	...
	##### Check php 5.x was installed or quit installer.
	...
	if  [[ "$PHPVER" == 5.* ]]; then
		...
		echo -e "\nPHP 5.x not installed. $PHPVER installed. Exiting installer. Please contact script admin"
		exit 1
	fi
